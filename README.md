...
- 複数 API 対応: 会計・人事労務・請求書・工数管理・販売の 5 つの freee API をサポート
...
対話式ウィザードが認証情報の設定、OAuth 認証、事業所選択を行います。
...
| Windows (Store 版) | `%LOCALAPPDATA%\Packages\Claude_pzs8sxrjxfjjc\LocalCache\Roaming\Claude\claude_desktop_config.json` |
...
## freee-cli (Dev Preview)

> Dev Preview: インターフェースは今後変更される可能性があります。SKILL（API リファレンス）との連動には未対応です。

ターミナルから freee API を直接呼び出せる curl ライクな CLI です。

```bash
npx freee-cli get /api/1/deals -s accounting -q limit=5
npx freee-cli post /api/1/deals -s accounting -d '{"issue_date":"2024-01-01", ...}'
npx freee-cli get /api/1/companies | jq '.companies[].display_name'
```

詳細は `npx freee-cli --help` を参照してください。
...
- [紹介記事: Public API を MCP 化するとき Agent Skill 併用が良さそう with freee-mcp](https://zenn.dev/him0/articles/766798ca1315e0)
