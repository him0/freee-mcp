...
- `freee_server_version` - サーバーバージョン確認
...
- バージョン確認: まず `VERSION.md` を読んでスキルのバージョンを確認し、`freee_server_version` でサーバーバージョンを確認してください。スキルのバージョンがサーバーより古い場合、スキルの情報が最新のサーバーに対応していない可能性があります。`claude skill upgrade freee-api-skill` または `npx skills add freee/freee-mcp` でスキルを更新してから再度お試しください。
