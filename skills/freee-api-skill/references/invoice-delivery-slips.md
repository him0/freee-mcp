...
| end_delivery_slip_date | query | いいえ | string(date) | 納品日の開始日 |
...
- **delivery_slips** **(必須)**: array[object]
  配列の要素:
    - **id** **(必須)**: integer(int64) - 納品書ID 例: `101` (最小: 1, 最大: 9223372036854776000)
    - **company_id** **(必須)**: integer(int64) - 事業所ID 例: `1` (最小: 1, 最大: 9223372036854776000)
    - **delivery_slip_number** **(必須)**: string - 納品書番号 例: `DEL-0000000001`
    - **subject** **(必須)**: string - 件名 例: `Webサイト製作費`
    - **template_id** (任意): integer(int64) - 帳票テンプレートID 例: `1` (最小: 1, 最大: 9223372036854776000)
    - **delivery_slip_date** **(必須)**: string(date) - 納品日 例: `2023-04-01` (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
...
- **delivery_slip_note** (任意): string - 備考 例: `備考
...
- **company_name** (任意): string - 自社名を上書きする場合に指定します。 例: `freee K.K.`
- **company_description** (任意): string - 自社説明を上書きする場合に指定します。 例: `freee K.K.`
...
- **delivery_slip** **(必須)**: object
  ...
  - **subject** **(必須)**: string - 件名 例: `Webサイト製作費`
  - **template_id** (任意): integer(int64) - 帳票テンプレートID 例: `1` (最小: 1, 最大: 9223372036854776000)
  - **template_name** (任意): string - 帳票テンプレート名 例: `標準の納品書テンプレート`
  - **delivery_slip_date** **(必須)**: string(date) - 納品日 例: `2023-04-01` (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
  - **issue_date** (任意): string(date) - 発生日 例: `2023-04-01` (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
...
- payment_typeがdirect_debitの場合、振替日に該当します。
 例: `2023-04-01` (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
  ...
  - **delivery_slip_note** **(必須)**: string - 備考 例: `備考`
  - **memo** **(必須)**: string - 社内メモ 例: `社内メモ`
  - **sending_status** **(必須)**: string - 送付ステータス（sent: 送付済み、 unsent: 送付待ち） (選択肢: sent, unsent) 例: `sent`
  - **payment_status** **(必須)**: string - 入金ステータス（unsettled: 入金待ち, settled: 入金済み） (選択肢: settled, unsettled) 例: `settled`
  - **cancel_status** **(必須)**: string - 取消済み（canceled: 該当する、 uncanceled: 該当しない） (選択肢: canceled, uncanceled) 例: `canceled`
...
- **delivery_slip** **(必須)**: object
  ...
  - **template_id** (任意): integer(int64) - 帳票テンプレートID 例: `1` (最小: 1, 最大: 9223372036854776000)
  - **template_name** (任意): string - 帳票テンプレート名 例: `標準の納品書テンプレート`
  - **delivery_slip_date** **(必須)**: string(date) - 請求日 例: `2023-04-01` (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
  - **issue_date** (任意): string(date) - 発生日 例: `2023-04-01` (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
  - **delivery_slip_note** **(必須)**: string - 備考 例: `備考`
  - **memo** **(必須)**: string - 社内メモ 例: `社内メモ`
  - **sending_status** **(必須)**: string - 送付ステータス（sent: 送付済み、 unsent: 送付待ち） (選択肢: sent, unsent) 例: `sent`
